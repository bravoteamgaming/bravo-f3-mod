...
if (missionNamespace getVariable["bravo_f3_mod_param_groupMarkers", -1] == 0) exitWith {}; // If 0 do nothing.

// If -1 automatically pick the correct value (see top)
if (missionNamespace getVariable["bravo_f3_mod_param_groupMarkers", -1] < 0) then { bravo_f3_mod_param_groupMarkers = [3, 1] select isClass(configFile >> "CfgPatches" >> "ace_main") };

// If Commander-Only markers are selected, exit here
if ((rank player) in ["PRIVATE", "CORPORAL", "SERGEANT"] && bravo_f3_mod_param_groupMarkers in [4,5]) exitWith {};

// Process list
{ _x spawn bravo_f3_mod_fnc_localGroupMarker } forEach (missionNamespace getVariable [format["bravo_f3_mod_var_groups%1",side group player],[]]);

// Set icons to show in-game also if chosen
if (bravo_f3_mod_param_groupMarkers == 2) then { 
...
if (!isNil "F_EH_GroupEnter") then { removeMissionEventHandler ["GroupIconOverEnter", bravo_f3_mod_EH_GroupEnter] };
if (!isNil "F_EH_GroupLeave") then { removeMissionEventHandler ["GroupIconOverEnter", bravo_f3_mod_EH_GroupLeave] };

bravo_f3_mod_EH_GroupEnter = addMissionEventHandler ["GroupIconOverEnter", {
	params [
	...
	if ((missionNamespace getVariable ["bravo_f3_mod_param_groupMarkers",0]) in [0,1,2,4]) exitWith {};
	...
	{	
		...
		_unitCol1 = [assignedTeam _x] call bravo_f3_mod_fnc_getIconColor;
		_unitCol2 = "FFFFFF";
		_unitCol3 = "888888";
		
		if (missionNamespace getVariable ["bravo_f3_mod_var_ShowInjured",false]) then {
	...
	if (profileNamespace getVariable ["bravo_f3_mod_var_ShowScore", false]) then {	
		// Add group score, casualties and sdr
		_pts = 0;

		units _group apply { _pts = _pts + score _x };
		_cas = _group getVariable ["bravo_f3_mod_var_casualtyCount", 0];
...
bravo_f3_mod_EH_GroupLeave = addMissionEventHandler ["GroupIconOverLeave", {
	params [
		"_is3D", "_group", "_waypointId",
		"_posX", "_posY",
		"_shift", "_control", "_alt"
	...
	_grpInfo = (missionNamespace getVariable [format["bravo_f3_mod_var_groups%1",side group player],[]]) select { _x#1 == groupId _group };
