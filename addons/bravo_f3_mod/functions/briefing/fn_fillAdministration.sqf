...
waitUntil{!isNil "bravo_f3_mod_var_timeOfDay"; !isNil "bravo_f3_mod_var_missionLoaded"};
...
private _startTime = missionNamespace getVariable ["bravo_f3_mod_var_timeOfDay", daytime];
...
_diaryText = _diaryText + format["<br/>Start Time: <font color='#00FFFF'>%1:%2</font>%7<br/>Sunrise: <font color='#FF0080'>%3:%4</font><br/>Sunset: <font color='#FF0080'>%5:%6</font><br/>",
		...
		if (missionNamespace getVariable ["bravo_f3_mod_param_timeMultiplier", 1] > 1) then { format[" - Accelerated: 1 Day is %1 Hour(s)", round (24 / bravo_f3_mod_param_timeMultiplier)] } else { "" }
	];
...
// If short view distance be default, then don't allow to override.
	...
	if (missionNamespace getVariable ["bravo_f3_mod_param_viewDistance", true]) then {
...
if ((player getVariable ["bravo_f3_mod_var_assignGear","r"]) in ["pp","ppc","pc","uav"]) then {
...
if (missionNamespace getVariable["bravo_f3_mod_param_radioMode", -1] >= 0) then {
	...
	_diaryText = _diaryText + format["<br/>Radio Mode: <font color='#00FFFF'>%1</font><br/>%2", 
		["All Units","Restricted"] select bravo_f3_mod_param_radioMode,
		["Any unit can join and leave any listed channel in Signals.","Only units with radio backpacks or units inside radio carrying vehicles may join channels listed in Signals."] select bravo_f3_mod_param_radioMode
	];
};
...
if (!isNil "bravo_f3_mod_param_respawn") then {
	if (bravo_f3_mod_param_respawn == 0) then {
		...
		if (bravo_f3_mod_param_respawn <= 10) then {
			_diaryText = _diaryText + format["<br/>LIMITED TICKETS: <font color='#00FFFF'>%1</font><br/>",bravo_f3_mod_param_respawn];
		} else {
			if (bravo_f3_mod_param_respawn in [30,60]) then {
				_diaryText = _diaryText + format["<br/>RESPAWN TIMER: <font color='#00FFFF'>%1</font> Seconds<br/>",bravo_f3_mod_param_respawn];
			} else {
				_diaryText = _diaryText + format["<br/>WAVE RESPAWN: <font color='#00FFFF'>%1</font> Minutes<br/>",[bravo_f3_mod_param_respawn,"MM"] call BIS_fnc_secondsToString];
...
if (missionNamespace getVariable["bravo_f3_mod_param_jipTeleport",0] > 0) then {
	switch (bravo_f3_mod_param_jipTeleport) do {
