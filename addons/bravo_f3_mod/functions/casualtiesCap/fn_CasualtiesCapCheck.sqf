...
if (isNil "bravo_f3_mod_param_CasualtiesCap") then { bravo_f3_mod_param_CasualtiesCap = 80 }; // Percentage of casualties.
if (isNil "bravo_f3_mod_param_CasMinToStart") then { bravo_f3_mod_param_CasMinToStart = 16 };	// Minimum players needed to start checking.
...
params["_grpstemp",						// Side (e.g. BLUFOR), or group name(s) as string array (e.g. ["mrGroup1","myGroup2"])
       ...
       ["_pc", bravo_f3_mod_param_CasualtiesCap],	// Percentage of casualties, if not used the global parameter will be taken instead.
...
if (isNil "bravo_f3_mod_fnc_updateCas") then { bravo_f3_mod_fnc_updateCas = compileFinal preprocessFileLineNumbers "bravo_f3_mod\casualtiesCap\fn_UpdateCasualties.sqf"; };
...
if (({ alive _x } count allPlayers) < bravo_f3_mod_param_CasMinToStart) then { 
	waitUntil { 
		sleep 30;
		diag_log text format ["[F3] INFO (f\casualtiesCap\f_CasualtiesCapCheck.sqf): Insufficient players (%1/%2)", ({ alive _x } count allPlayers), bravo_f3_mod_param_CasMinToStart, _grpstemp];
		({ alive _x } count allPlayers) >= bravo_f3_mod_param_CasMinToStart;
	};
};
...
while {true} do {
	...
	// Calculate how many units in the groups are still alive
	{
		...
		if ((missionNamespace getVariable [format["bravo_f3_mod_var_casualtyCount_%1",side _grp], 0]) > _pc) then {_rawLimit = true};
	} forEach _grps;
