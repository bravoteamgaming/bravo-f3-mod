...

{
	...
	if (_side == _sideVar) exitWith {
		private _casVar = (missionNamespace getVariable [format["bravo_f3_mod_var_casualtyCount_%1",_sideVar], 0]) + 1;
		private _casCap = missionNamespace getVariable["bravo_f3_mod_param_CasualtiesCap", 80];
		missionNamespace setVariable [format["bravo_f3_mod_var_casualtyCount_%1", _sideVar], _casVar, true];
		...
		if (missionNamespace getVariable["bravo_f3_mod_param_CasualtiesShow", false]) then { _markerVar setMarkerText format["Casualties: %1", _casVar] };
		
		if (_group isEqualType grpNull) then { _group setVariable ["bravo_f3_mod_var_casualtyCount", (_group getVariable ["bravo_f3_mod_var_casualtyCount",0]) + 1, true]; };
