...
// Check Game Type
if (getText ((getMissionConfig "Header") >> "gameType") == "") then { ["f_debug.sqf","Game Type has not been defined!"] call bravo_f3_mod_fnc_logIssue }; 

// Check for BIS Revive
if (getMissionConfigValue ["ReviveMode",0] > 0) then { ["f_debug.sqf","BIS Revive is Active, cannot use FAROOQ Medical."] call bravo_f3_mod_fnc_logIssue }; 

// Check for mines
if (count allMines > 5 && { _x getUnitTrait 'explosiveSpecialist' } count (playableUnits + switchableUnits) <= 0) then { ["f_debug.sqf","Mines present but no Explosives Experts detected!"] call bravo_f3_mod_fnc_logIssue }; 

// Check for unassigned completions
if (count ((allMissionObjects "ModuleTaskSetState_F") select { _x getVariable "State"  in ["CREATED","ASSIGNED"] }) > 0) then { ["f_debug.sqf","Task Status change is not Complete! Not set to 'Succeed' or 'Failure' is this correct?"] call bravo_f3_mod_fnc_logIssue };

// Missing Group Markers
if ({ !(groupID group _x in ((missionNamespace getVariable ["bravo_f3_mod_var_allGroups",[["","",""]]]) apply { _x#2 }))} count (playableUnits + switchableUnits) > 0) then {
	["groups.sqf",format["%1 units have no group ID assigned: %2",
		{ !(groupID group _x in ((missionNamespace getVariable ["bravo_f3_mod_var_allGroups",[["","",""]]]) apply { _x#2 }))} count (playableUnits + switchableUnits),
		(playableUnits + switchableUnits) select { !(groupID group _x in ((missionNamespace getVariable ["bravo_f3_mod_var_allGroups",["","",""]]) apply { _x#2 }))}
		]
	] call bravo_f3_mod_fnc_logIssue
}; 

// Warn if the file wasn't cleared.
if (count (missionNamespace getVariable ["bravo_f3_mod_var_allGroups",[]]) > 25) then { ["mission\groups.sqf",format["%1 groups are listed! Remove unused groups from 'groups.sqf'", count bravo_f3_mod_var_allGroups]] call bravo_f3_mod_fnc_logIssue };
	...
	if (["module", format ["%1",typeOf _x]] call BIS_fnc_inString) then {
		...
		if ("id" in allVariables _module) then {
			...
			if (_moduleID == "") then {
				...
				["f_debug.sqf",format["<marker name='%3'>%1</marker> in %2 has a no ID, assign one.<br/>",_moduleID,typeOf _module,_mkr]] call bravo_f3_mod_fnc_logIssue;
			};
			...
			if (_moduleID in _arr) then {
				...
				_mkr setMarkerSizeLocal [0.8,0.8];
				_mkr setMarkerTypeLocal "mil_dot";
				["f_debug.sqf",format["<marker name='%3'>%1</marker> in %2 has a duplicate ID! IDs cannot be the same.<br/>",_moduleID,typeOf _module,_mkr]] call bravo_f3_mod_fnc_logIssue;
				...
				switch (_module getVariable "owner") do {
					case 1: {
						if (count synchronizedObjects _module == 0) then {
							...
							_mkr setMarkerSizeLocal [0.8,0.8];
							_mkr setMarkerTypeLocal "mil_dot";
							["f_debug.sqf",format["<marker name='%2'>Task %1</marker> has owner assigned as group but no synchronized units.",_moduleID,_mkr]] call bravo_f3_mod_fnc_logIssue;
						};
					};
					case 4: {
						if ({side _x == west} count (playableUnits + switchableUnits) == 0) then {
							...
							_mkr setMarkerSizeLocal [0.8,0.8];
							_mkr setMarkerTypeLocal "mil_dot";
							["f_debug.sqf",format["<marker name='%2'>Task %1</marker> has owner assigned as BLUFOR but no playable units.",_moduleID,_mkr]] call bravo_f3_mod_fnc_logIssue;
						};
					};
					case 5: {
						if ({side _x == east} count (playableUnits + switchableUnits) == 0) then {
							...
							_mkr setMarkerSizeLocal [0.8,0.8];
							_mkr setMarkerTypeLocal "mil_dot";
							["f_debug.sqf",format["<marker name='%2'>Task %1</marker> has owner assigned as OPFOR but no playable units.",_moduleID,_mkr]] call bravo_f3_mod_fnc_logIssue;
						};
					};
					case 6: {
						if ({side _x == resistance} count (playableUnits + switchableUnits) == 0) then {
							...
							_mkr setMarkerSizeLocal [0.8,0.8];
							_mkr setMarkerTypeLocal "mil_dot";
							["f_debug.sqf",format["<marker name='%2'>Task %1</marker> has owner assigned as INDFOR but no playable units.",_moduleID,_mkr]] call bravo_f3_mod_fnc_logIssue;
						};
					};
				};
		};
		...
		switch (toUpper((triggerActivation _x) select 0)) do {
			case "WEST": {
				if ({side _x == west} count allUnits == 0) then {
					...
					_mkr setMarkerSizeLocal [0.8,0.8];
					_mkr setMarkerTypeLocal "mil_dot";
					["f_debug.sqf",format["<marker name='%1'>Trigger (%2)</marker> has activation type as BLUFOR but no units present.",_mkr,vehicleVarName _x]] call bravo_f3_mod_fnc_logIssue;
				};
			};
			case "EAST": {
				if ({side _x == east} count allUnits == 0) then {
					...
					_mkr setMarkerSizeLocal [0.8,0.8];
					_mkr setMarkerTypeLocal "mil_dot";
					["f_debug.sqf",format["<marker name='%1'>Trigger (%2)</marker> has activation type as OPFOR but no units present.",_mkr,vehicleVarName _x]] call bravo_f3_mod_fnc_logIssue;
				};
			};
			case "GUER": {
				if ({side _x == resistance} count allUnits == 0) then {
					...
					_mkr setMarkerSizeLocal [0.8,0.8];
					_mkr setMarkerTypeLocal "mil_dot";
					["f_debug.sqf",format["<marker name='%1'>Trigger (%2)</marker> has activation type as INDFOR but no units present.",_mkr,vehicleVarName _x]] call bravo_f3_mod_fnc_logIssue;
				};
			};
		};
	};
...
// Display Issues List
if (count (missionNamespace getVariable ["bravo_f3_mod_var_missionLog",[]]) > 0) then { 
	...
	bravo_f3_mod_var_missionLog sort true;
	...
	} forEach bravo_f3_mod_var_missionLog;
