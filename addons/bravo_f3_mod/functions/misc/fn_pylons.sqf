...
if (isNil "bravo_f3_mod_pylon_disabled") then { bravo_f3_mod_pylon_disabled = false }; // Use this to exclude certain players / roles
if (isNil "bravo_f3_mod_pylon_excludeMags") then { bravo_f3_mod_pylon_excludeMags = ["rhs_mag_DAGR_8","rhs_mag_AGM114L_2","rhs_mag_AGM114K_2","rhs_mag_AGM114M_2","rhs_mag_AGM114N_2","rhs_mag_M151_7","rhs_mag_M229_7","PylonRack_3Rnd_ACE_Hellfire_AGM114K","PylonRack_3Rnd_ACE_Hellfire_AGM114N","PylonRack_3Rnd_ACE_Hellfire_AGM114L","ace_hot_1_PylonRack_3Rnd","ace_hot_2_PylonRack_3Rnd","ace_hot_2MP_PylonRack_3Rnd","ace_hot_3_PylonRack_3Rnd"] }; // Exclude any pylons from CfgMagazines here (must be array!).
if (isNil "bravo_f3_mod_pylon_rearmMode") then { bravo_f3_mod_pylon_rearmMode = 1 }; // Rearm mode: 0 = Time Only, 1 = Vehicle Needed after FreeTime
if (isNil "bravo_f3_mod_pylon_freeTime") then { bravo_f3_mod_pylon_freeTime = 300 }; // Grace period for free changes (5 minutes).

if (_vType isEqualType objNull) then { _vType = typeOf _vType };

if (bravo_f3_mod_pylon_disabled || count (_vType getCompatiblePylonMagazines 1) < 1) exitWith {}; // Nothing to do!
...
missionNamespace setVariable [format['bravo_f3_mod_var_pylon_%1', _vType], []]; // Store vehicles starting loadout.
missionNamespace setVariable [format['bravo_f3_mod_var_pylon_%1_owner', _vType], []];

if !(bravo_f3_mod_pylon_excludeMags isEqualType []) then { bravo_f3_mod_pylon_excludeMags = [] }; // Exclude list must be array!
...
fnc_pylon_setPylons = {
	...
	private _disabled = missionNamespace getVariable ["bravo_f3_mod_pylon_disabled", false];
	private _timeLimit = missionNamespace getVariable ["bravo_f3_mod_pylon_freeTime", 300];
	private _rearmMode = missionNamespace getVariable ["bravo_f3_mod_pylon_rearmMode", 0];
	private _loadout = missionNamespace getVariable [ format["bravo_f3_mod_var_pylon_%1", _type], []];
	private _owner = missionNamespace getVariable [ format["bravo_f3_mod_var_pylon_%1_owner", _type], []];
...
_pylonRecord = _pylonRecord + format["The following allows a Virtual Template to be built and applied to a vehicle in-game. Set up the Template during briefing and then apply it to the vehicle when in the mission. You will have <font color='#00FFFF'>%1 Minutes</font> to apply a Template to the vehicle. After which a <font color='#00FFFF'>Support Vehicle</font> must be present to apply any changes.<br/><br/>", round (bravo_f3_mod_pylon_freeTime / 60)];
...

{
	...
	_pylonOwners = _pylonOwners + format[" | <execute expression=""systemChat '%3 (%5) owner: Pilot'; bravo_f3_mod_var_pylon_%1_owner set [%4,[]];"">Pilot%2</execute>", _vType, ["*",""] select _isTurret, configName _x, _forEachIndex, _pylonStr];
	_pylonOwners = _pylonOwners + format[" | <execute expression=""systemChat '%3 (%5) owner: Gunner'; bravo_f3_mod_var_pylon_%1_owner set [%4,[0]];"">Gunner%2</execute>", _vType, ["","*"] select _isTurret, configName _x, _forEachIndex, _pylonStr];
	_pylonOwners = _pylonOwners + format[" | <execute expression=""systemChat '%3 (%5) owner: Default'; bravo_f3_mod_var_pylon_%1_owner set [%4,0];"">Clear</execute> - <font color='#80FF00'>%3</font><br/>", _vType, "", configName _x, _forEachIndex, _pylonStr];
...
_pylonRecord = _pylonRecord + format["<execute expression=""bravo_f3_mod_var_pylon_%1 = ['']; systemChat '[%2] Template Selected: Empty';"">Empty</execute><br/><font size='12'>No Weapons</font>", _vType, _vName];

{
	if !(toUpper (configName _x) in ['EMPTY','NONE']) then {
		_pylonRecord = _pylonRecord + format["<br/><br/><execute expression=""bravo_f3_mod_var_pylon_%1 = getArray (configFile >> 'CfgVehicles' >> '%1' >> 'Components' >> 'TransportPylonsComponent' >> 'Presets' >> '%2' >> 'attachment'); systemChat '[%4] Template Loaded: %3';"">%3</execute><br/>", _vType, configName _x, getText (_x >> "displayName"), _vName];
...
// For each mag, add 
{
	...
	{
		private _pylonNo = _forEachIndex + 1;
		if (_mag in _x) then { _pylonRecord = _pylonRecord + format[" | <font size='12'> <execute expression=""systemChat '[%5] Template Pylon%1: %2'; bravo_f3_mod_var_pylon_%6 set [%3,'%4']"">%1</execute></font>", if (_pylonNo > 9) then { _pylonNo } else { '0' + str _pylonNo }, _name, _forEachIndex, _mag, _vName, _vType] };
	} forEach _pylonMags;
...
} forEach (_magazines - bravo_f3_mod_pylon_excludeMags);
