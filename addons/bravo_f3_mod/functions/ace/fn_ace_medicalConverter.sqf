...
waitUntil{!isNil "bravo_f3_mod_var_medical_level"};

if (bravo_f3_mod_var_medical_level == 0 || !isClass(configFile >> "CfgPatches" >> "ace_main")) exitWith {}; // ACE not running so don't do anything more!
...
if (missionNamespace getVariable["bravo_f3_mod_param_debugMode",0] == 1) then { 
	["fn_ace_medicalConverter.sqf",format["Converting medical (ADV:%5) for %2 (%1 - F%3 M%4)", _unit, typeOf _unit, _cntFAK, _cntMediKit, _advMedical],"INFO"] call bravo_f3_mod_fnc_logIssue;
};
