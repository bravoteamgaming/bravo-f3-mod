...
if (missionNamespace getVariable["bravo_f3_mod_param_debugMode",0] == 1) then { 
	["ace_clientInit.sqf",format["Starting for %1",player],"INFO"] call bravo_f3_mod_fnc_logIssue;
};

// Wait for gear assignation to take place.
_timeUntil = time + 1;
waitUntil{(player getVariable ["bravo_f3_mod_var_assignGear_done", false]) || time > _timeUntil};

// Wait for ACE to finish setting up.
waitUntil{missionNamespace getVariable ["ace_common_settingsinitfinished",false]};

_typeofUnit = player getVariable ["bravo_f3_mod_var_assignGear", "r"];
...
player setVariable ["bravo_f3_mod_var_ACEclientInitDone",true];