...
// Is the variable set?
if (isNil "bravo_f3_mod_param_jipTeleport") then { bravo_f3_mod_param_jipTeleport = 0 };

// Exit if disabled
if (missionNamespace getVariable["bravo_f3_mod_param_jipTeleport", 0] == 0) exitWith {};

// Set defaults.
params [
	["_spawnAction", (switch (bravo_f3_mod_param_jipTeleport) do { case 1; case 3: { true }; default { false } }) ],
	["_spawnFlag", (switch (bravo_f3_mod_param_jipTeleport) do { case 2; case 3: { true }; default { false } }) ]
];
...
if _spawnFlag then {
	...
	if (_flagMarker in allMapMarkers) then {
		...
		bravo_f3_mod_obj_spawnFlag addAction ["<t color='#80FF00'>Spawn on Team</t>",bravo_f3_mod_fnc_teleportPlayer];
	} else {
		if (_flagMarker != "respawn_civilian") then {
			["f_teleportOption.sqf",format["No respawn marker found for %1.",side (group player)]] call bravo_f3_mod_fnc_logIssue;
		};
	};
};
...
if _spawnAction then {
	...
	if (missionNamespace getVariable ["f_param_jipRespawn", false] && isNil "bravo_f3_mod_param_jipEHID") then {
		bravo_f3_mod_param_jipEHID = player addEventHandler ["Respawn",{spawn bravo_f3_mod_fnc_teleportOption;}];
	};
	...
	if (isClass(configFile >> "CfgPatches" >> "ace_main")) then {
		...
		ace_jip_action = ['JIP','Spawn on Team','\a3\Ui_f\data\Map\Diary\Icons\unitgroupplayable_ca.paa',bravo_f3_mod_fnc_teleportPlayer,{true}] call ace_interact_menu_fnc_createAction;
		...
		_teleAction = player addAction ["<t color='#80FF00'>Spawn on Team</t>",bravo_f3_mod_fnc_teleportPlayer];
		sleep _timer;
