...
if (isNil "bravo_f3_mod_var_setParams") then {
	...
	if (_paramsLoaded) then {
		...
		if isNil "bravo_f3_mod_param_debugMode" then { missionNamespace setVariable ["bravo_f3_mod_param_debugMode",0, true] };
		if (profileName == missionNamespace getVariable["bravo_f3_mod_debugProfile","2600K"]) then { missionNamespace setVariable ["bravo_f3_mod_param_debugMode",1]; };
			
		// Medical Settings
		private _medicalVar = missionNamespace getVariable["bravo_f3_mod_param_medical",-1];
		
		// ACE overrides all if present
		if (isClass(configFile >> "CfgPatches" >> "ace_main")) then {
			missionNamespace setVariable ["bravo_f3_mod_var_medical_level",2,true];
		} else {
			switch (_medicalVar) do {
				case 0: {
					// Vanilla Selected
					missionNamespace setVariable ["bravo_f3_mod_var_medical_level",0,true];
				};
				case 1: {
					...
					if (getMissionConfigValue ["ReviveMode",0] != 0) then {
						...
						missionNamespace setVariable ["bravo_f3_mod_var_medical_level",1,true];
					};
				};
				default {
		...
		if (isNil "bravo_f3_mod_param_radios") then {
			if ("task_force_radio" in activatedAddons) then {
				missionNamespace setVariable ["bravo_f3_mod_param_radios",1,true]; // TFAR Radio
			} else { 
				if ("acre_main" in activatedAddons) then {
					missionNamespace setVariable ["bravo_f3_mod_param_radios",2,true]; // ACRE Radio
				} else { 
					missionNamespace setVariable ["bravo_f3_mod_param_radios",0,true]; // Vanilla Radio
				};
			};
		};
		...
		if (fileExists "f3\f3_mission_groups.sqf") then {
			call compile preprocessFileLineNumbers "f3\f3_mission_groups.sqf";
		};
		
		// Older FW Support
		if (!isNil "_grpBLU") then { bravo_f3_mod_var_groupsWEST = _grpBLU };
		if (!isNil "_grpOPF") then { bravo_f3_mod_var_groupsEAST = _grpOPF };
		if (!isNil "_grpIND") then { bravo_f3_mod_var_groupsGUER = _grpIND };
		if (!isNil "_grpCIV") then { bravo_f3_mod_var_groupsCIVILIAN = _grpCIV };
		
		{ missionNamespace setVariable [format["bravo_f3_mod_var_groups%1", _x], missionNamespace getVariable [format["bravo_f3_mod_var_groups%1", _x],[]], true] } forEach [west, east, independent, civilian];
		
		// Let the client know params are set.
		if (_paramsLoaded) then {
			missionNamespace setVariable ["bravo_f3_mod_var_setParams",true,true]; 
			
			if (missionNamespace getVariable["bravo_f3_mod_param_debugMode",0] == 1) then { 
				diag_log text format["[F3] DEBUG (fn_processParamsArray.sqf): bravo_f3_mod_var_setParams: %1",f_var_setParams];
			};
		};
	};
};