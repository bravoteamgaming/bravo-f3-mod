...
if (isNil "bravo_f3_mod_var_mapClickTeleport_Uses") then {bravo_f3_mod_var_mapClickTeleport_Uses = _uses};

// False: everyone can TP. True: Only group leaders can TP and will move their entire group.
if (isNil "bravo_f3_mod_var_mapClickTeleport_GroupTeleport") then {bravo_f3_mod_var_mapClickTeleport_GroupTeleport = _groupTP};

// If > 0 map click TP will act as a HALO drop and automatically assign parachutes to units
if (isNil "bravo_f3_mod_var_mapClickTeleport_Height") then {bravo_f3_mod_var_mapClickTeleport_Height = _dropHeight};

// Allow all group leaders access to Teleport.
if (bravo_f3_mod_var_mapClickTeleport_GroupTeleport && player != leader group player) exitWith {};
...
if (_skipAction) exitWith { [] spawn bravo_f3_mod_fnc_mapClickTeleportUnit };

_string = if (bravo_f3_mod_var_mapClickTeleport_Height == 0) then {"Fast Travel"} else {"HALO Drop"};
...
private _tel = player createDiaryRecord ["Diary", [_string, format["
	...
	_string,
	if (bravo_f3_mod_var_mapClickTeleport_Uses > 1) then { format[", this option may be used <font color='#FF0080'>%1</font color> times.", bravo_f3_mod_var_mapClickTeleport_Uses] } else { ", once used this action will be removed." },
	if (_timeLimit > 0) then { format["<br/><br/>This option will EXPIRE after <font color='#FF0080'>%1</font color> seconds.", _timeLimit] } else { "" },
	if (bravo_f3_mod_var_mapClickTeleport_GroupTeleport) then { "<br/><br/>Using this command will also move your entire team, so ensure they are ready!" } else { "<br/><br/>Using this command will move only your unit, it will not affect other players." },
	if (bravo_f3_mod_var_mapClickTeleport_Height > 0) then { format["<br/><br/>You will be dropping from a height of around <font color='#FF0080'>%1m</font color>.", bravo_f3_mod_var_mapClickTeleport_Height] } else { "<br/><br/>If you are inside a vehicle, that vehicle will also be transported." }
]]];
...
bravo_f3_mod_var_mapClickTeleport_textAction = format["<t color='#80FF00'>%1</t>",_string];
...
if (isClass(configFile >> "CfgPatches" >> "ace_main")) then {
		...
		ace_par_action = ['PAR',format["%1",_string],['\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\run_ca.paa','\A3\ui_f_orange\data\cfgTaskTypes\airdrop_ca.paa'] select (bravo_f3_mod_var_mapClickTeleport_Height > 0),{[] spawn bravo_f3_mod_fnc_mapClickTeleportUnit},{true}] call ace_interact_menu_fnc_createAction;
	...
	if (_timeLimit > 0) then {
		sleep _timeLimit;
		player removeAction bravo_f3_mod_mapClickTeleportAction;
	};
};