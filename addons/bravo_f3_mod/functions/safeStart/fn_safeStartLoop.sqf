...
while {bravo_f3_mod_param_safe_start > 0} do {

	// If mission timer has been terminated by admin briefing, simply exit
	if (bravo_f3_mod_param_safe_start < 0) exitWith {};

	// Broadcast remaining time to players
	["SafeStart",[format["Time Remaining: %1 min",bravo_f3_mod_param_safe_start]]] remoteExec ["bis_fnc_showNotification",0];

	uiSleep 60; // Sleep 60 seconds

	// Reduce the mission timer by one
	missionNamespace setVariable ["bravo_f3_mod_param_safe_start",bravo_f3_mod_param_safe_start - 1,true];
};
...
if (bravo_f3_mod_param_safe_start == 0) then {
		...
		[false] remoteExec ["bravo_f3_mod_fnc_safety",0,true];
};