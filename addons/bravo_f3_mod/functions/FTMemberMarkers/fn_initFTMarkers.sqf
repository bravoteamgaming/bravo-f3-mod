...
if (isNil "bravo_f3_mod_var_ShowFTMarkers") then { bravo_f3_mod_var_ShowFTMarkers = !(isClass(configFile >> "CfgPatches" >> "ace_main")) };
...
_f_fnc_drawFTMarker = {	
	...
	
	{
		...
		if !(bravo_f3_mod_var_ShowFTMarkers) exitWith {};
		...
		if (missionNamespace getVariable ["bravo_f3_mod_var_ShowInjured",true]) then {
		...
		if  (leader _x == _x && (missionNamespace getVariable ["bravo_f3_mod_param_groupMarkers",0]) in [1,2,3]) then { _iconShape = "" };
		...
		if (_iconShape != "") then {
			...
			[
				...
				1,
				0.035,
				(missionNamespace getVariable ["bravo_f3_mod_FONT_NAMETAGS","PuristaBold"]),
				'right'
			]
		};
...
if (!isNil "bravo_f3_mod_eh_ftmap") then { findDisplay 12 displayCtrl 51 ctrlRemoveEventHandler ["Draw", f_eh_ftmap] };
