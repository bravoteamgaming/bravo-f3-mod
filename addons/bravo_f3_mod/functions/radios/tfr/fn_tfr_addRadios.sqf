...
// DECLARE VARIABLES
private _typeofUnit = player getVariable ["bravo_f3_mod_var_assignGear", "NIL"];
	...
	if (!bravo_f3_mod_radios_settings_disableAllRadios) then {
		...
		if (_typeofUnit in _fRadiosPersonal || "all" in _fRadiosPersonal || (_isLeader && "leaders" in _fRadiosPersonal)) then {
			if (!isClass (configFile >> "CfgWeapons" >> _radio2)) exitWith {
				["fn_tfr_addRadios.sqf",format["Invalid Radio: %1", _radio2],"ERROR"] call bravo_f3_mod_fnc_logIssue;
			};
			...
			if (_typeofUnit in _fRadiosRifleman || "all" in _fRadiosRifleman || (_isLeader && "leaders" in _fRadiosRifleman)) then {
				if (!isClass (configFile >> "CfgWeapons" >> _radio3)) exitWith {
					["fn_tfr_addRadios.sqf",format["Invalid Radio: %1", _radio3],"ERROR"] call bravo_f3_mod_fnc_logIssue;
				};
		...
		if (_typeofUnit in _fRadiosLongRange || "all" in _fRadiosLongRange || (_isLeader && "leaders" in _fRadiosLongRange)) then {
			if (_isLeader) then {
				if (!isClass (configFile >> "CfgWeapons" >> _radio1)) exitWith {
					["fn_tfr_addRadios.sqf",format["Invalid Radio: %1", _radio1],"ERROR"] call bravo_f3_mod_fnc_logIssue;
				};
		...
		if (_hasRadio && !isNil "bravo_f3_mod_tfar_localSRfreq") then {
			[] spawn {
				...
				player groupChat format[ "Radio Channel #1 set to %1Mhz (%2)", bravo_f3_mod_tfar_localSRfreq, groupId (group player)];
			};
		};
	};
};