...
if (isNil "bravo_f3_mod_param_timeOfDay") exitWith { ["fn_tidyGear.sqf", "No TOD Parameter set - Exiting", "INFO"] call bravo_f3_mod_fnc_logIssue };

waitUntil{!isNil "bravo_f3_mod_var_timeOfDay" || time > 0};

["fn_tidyGear.sqf", format["Tidy Gear Running - Time of Day: %1", bravo_f3_mod_var_timeOfDay], "INFO"] call bravo_f3_mod_fnc_logIssue;
...
if (isNil "bravo_f3_mod_var_timeOfDay") then {
	bravo_f3_mod_var_timeOfDay = floor daytime;
};

// Skip script on dusk, dawn or 1hr before dusk as light will change.
if (isNil "_forced") then {
	if (bravo_f3_mod_var_timeOfDay == _sunrise || bravo_f3_mod_var_timeOfDay == _sunset || bravo_f3_mod_var_timeOfDay == (_sunset - 1)) exitWith {};
...
["fn_tidyGear.sqf", format["Gear switched, removed %2 items", _itemCount], "INFO"] call bravo_f3_mod_fnc_logIssue;