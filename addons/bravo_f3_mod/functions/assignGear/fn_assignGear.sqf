...
if (isNil "bravo_f3_mod_param_backpacks") then {bravo_f3_mod_param_backpacks = 1};
...
_unit setVariable ["bravo_f3_mod_var_assignGear",_typeofUnit, true];
...
if (isNil format["bravo_f3_mod_var_%1_gear_smokeTH",_side]) then { [format["bravo_f3_mod_var_%1_gear_smokeTH",_side],["_smokegrenade","_smokegrenade1","_smokegrenade2","_smokegrenade3","_smokeGrenadeAlt","_smokeGrenadeAlt1","_smokeGrenadeAlt2"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_flareTH",_side]) then { [format["bravo_f3_mod_var_%1_gear_flareTH",_side],["_chem","_chem1","_chem2","_chem3","_chemAlt","_chemAlt1","_chemAlt2"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_smokeGL",_side]) then { [format["bravo_f3_mod_var_%1_gear_smokeGL",_side],["_glsmoke","_glsmoke1","_glsmoke2","_glsmoke3","_glsmokealt","_glsmokealt1","_glsmokealt2"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_flareGL",_side]) then { [format["bravo_f3_mod_var_%1_gear_flareGL",_side],["_glflare","_glflare1","_glflare2","_glflare3","_glflarealt","_glflarealt1","_glflarealt2"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_grenaGL",_side]) then { [format["bravo_f3_mod_var_%1_gear_grenaGL",_side],["_glmag"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_grenade",_side]) then { [format["bravo_f3_mod_var_%1_gear_grenade",_side],["_grenade","_grenade1","_grenade2","_grenade3","_grenadealt","_grenadealt1","_grenadealt2"]] call _f_fnc_parseGear; };	
if (isNil format["bravo_f3_mod_var_%1_gear_glasses",_side]) then { [format["bravo_f3_mod_var_%1_gear_glasses",_side],"_baseGlasses","CfgGlasses"] call _f_fnc_parseGear; };
...
if (_isMan && !_skipCheck) then {
	...
	if (handgunWeapon _unit != "") then {
		...
		if (_unitMags arrayIntersect _configMags isEqualTo []) then {
			["fn_assignGear.sqf",format["Unit %1 %2_%3 has no additional magazines for side-arm '%4'.",_unit,_side,_typeofUnit,handgunWeapon _unit]] call bravo_f3_mod_fnc_logIssue;
			_unit addMagazines [(_configMags select 0),2];
		};
	};
	
	if (primaryWeapon _unit == "") then {
		["fn_assignGear.sqf",format["Unit %1 %2_%3 has no primary weapon!",_unit,_side,_typeofUnit]] call bravo_f3_mod_fnc_logIssue;
	} else {
		_configMags = ([primaryWeapon _unit] call BIS_fnc_compatibleMagazines) apply { toLower _x };
		
		if (_unitMags arrayIntersect _configMags isEqualTo []) then {
			["fn_assignGear.sqf",format["Unit %1 %2_%3 has no additional magazines for primary '%4'.",_unit,_side,_typeofUnit,primaryWeapon _unit]] call bravo_f3_mod_fnc_logIssue;
			_unit addMagazines [(_configMags select 0),2];
			reload _unit;
		};
	};
	...
	if !(_unit canAdd "ItemMap") then {
		["fn_assignGear.sqf",format["Gear for %1 %2_%3 slot is overfilled.",_unit,_side,_typeofUnit]] call bravo_f3_mod_fnc_logIssue;
	};
	...
	if (!isNil "_uavterminal" && _typeofUnit == "uav") then {
		...
		if (_uavIndex >= 0) then {
			if !(_uavterminal isEqualTo (_uavList select _uavIndex select 1)) then {
				["fn_assignGear.sqf",format["UAV Terminal for %1 %2_%3 didn't match their side.",_unit,_side,_typeofUnit]] call bravo_f3_mod_fnc_logIssue;
...
if (isNil "_carbine") then { //_carbine should exist unless no faction has been called
	["fn_assignGear.sqf",format["%1 is not defined for side '%2'",_typeofUnit, _side]] call bravo_f3_mod_fnc_logIssue;
};