	...
	
	{
		...
		
		{
			if (getNumber(configFile >> "CfgWeapons" >> _x >> "itemInfo" >> "type") > 0) then {
				...
				private _allowedArray = [primaryWeapon _unit,getNumber(configFile >> "CfgWeapons" >> _x >> "itemInfo" >> "type")] call bravo_f3_mod_fnc_compatibleItems;
				...
				["f_assignGear_attachments.sqf",format["Invalid attachment for %1_%2: '%3'",_side,_typeofUnit,_x],"ERROR"] call bravo_f3_mod_fnc_logIssue;
			};
		} forEach _attachment;

		if (count _attItems > 0) then {
			_unit addPrimaryWeaponItem (_attItems select 0);
			if (isPlayer _unit && isNil format["bravo_f3_mod_var_%1_%2_gear%3",_side,_typeofUnit,_attType]) then { 
				missionNamespace setVariable [format["bravo_f3_mod_var_%1_%2_gear%3",_side,_typeofUnit,_attType],_attItems,true];
...
if (handgunWeapon _unit != "") then {
	...
	// loop trough the attachments and add them to the weapon
	{
		if (isClass (configFile >> "CfgWeapons" >> _x)) then {
			...
			["fn_addWeapon.sqf",format["Invalid weapon for %1_%2: '%3'.",_side,_typeofUnit,_x],"ERROR"] call bravo_f3_mod_fnc_logIssue;
		};
